    ...
    var regs = [
        {
            ...
            reg: new RegExp(
                '^@botannick\\s+(?:est[\\s-]ce\\s+que\\s+)?tu\\s+(?:peux|pourrais?|pourras|veux\\s+bien|voudrais?\\s+bien|accepte\\s*de)\\s+me\\s+(' + infinitives.join('|') + ')\\s+(pour|de|d[\\s\']|qu[e\'])\\s*(.+)\\s*dans\\s+([0-9]+\\s*(?:h|heures?|ms?|mns?|mins?|minutes?|js?|jours?))',
                'i'
            )
        },
        {
            ...
            reg: new RegExp(
                '^@' + config.bot.credentials.username + '\\s+(' + imperatives.join('|') + ')\\s+moi\\s+(pour|de|d[\\s\']|qu[e\'])\\s*(.+)\\s*dans\\s+([0-9]+\\s*(?:h|heures?|ms?|mns?|mins?|minutes?|js?|jours?))',
                'i'
            )
        }
    ];
